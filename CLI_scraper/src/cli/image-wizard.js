...
async function flowNewTask(rl) {
  ...
  // Step 4 — size. The fetcher strips each URL's native suffix before
  // appending the chosen one, so any valid Google size spec works. `-k-no`
  // means no-crop (preserve aspect); drop it if you want square crops.
  const sizeIdx = await pickOne(rl, 'Step 4/7 — Image size:', [
    's1024-w1024-h1024-k-no  (1024² no-crop, balanced)',
    'w800-h600               (800×600)',
    'w1600-h1200             (high-res)',
    's400                    (thumbnail)',
    'raw                     (largest available, s0)',
    'custom...',
  ]);
  const sizeOptions = [
