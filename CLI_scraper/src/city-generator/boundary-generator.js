...
class BoundaryGenerator {
  ...
  /**
   * Query Overpass API with mirror failover + throttling.
   * Tries each mirror in order; waits at least _minGapMs between any two
   * outgoing requests to avoid self-inflicted rate limits.
   * @private
   */
  ...
  _queryOverpassOnce(query, overpassUrl) {
    return new Promise((resolve, reject) => {
      ...
      const options = {
        ...
        headers: {
          ...
          // Kumi / private.coffee reject requests without a UA string.
          'User-Agent': 'gmaps-crab/1.0 (+boundary-generator; contact via repo)'
        },
        timeout: 60000
      };
