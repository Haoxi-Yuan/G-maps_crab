...
async function batchSearchPOIs(browser, points, categories, options = {}, progressCallback = null) {
  ...
  const saveInterval = options.saveInterval || 50; // Save every 50 searches by default
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      for (let j = 0; j < categories.length; j++) {
        ...
        if (incrementalSaveFile && searchCount % saveInterval === 0) {
          ...
          try {
            fs.writeFileSync(incrementalSaveFile, JSON.stringify(partialResults, null, 2), 'utf8');
