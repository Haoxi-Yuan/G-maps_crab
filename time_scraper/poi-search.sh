...
stop_background() {
  local pattern="poi-searcher-api"
  if [ -n "$STOP_CITY" ]; then
    pattern="poi-searcher-api.*${STOP_CITY}\|${STOP_CITY}.*poi-searcher"
    echo "Stopping POI search for: $STOP_CITY"
  else
    echo "Stopping ALL POI search processes"
  fi
